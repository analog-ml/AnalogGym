...
register(
    id=env_id,
    entry_point="AMP_NMCF:AMPNMCFEnv",  # update this entry_point for the new circuit
...
if save == True:
    ...
    # save memory
    with open(
        f"./saved_memories/memory_{CktGraph().__class__.__name__}_{date}_noise={noise_type}_reward={best_reward:.2f}_{GNN().__class__.__name__}_rew_eng={rew_eng}.pkl",
        "wb",
    ) as memory_file:
        pickle.dump(memory, memory_file)

    np.save(
        f"./saved_memories/rews_buf_{CktGraph().__class__.__name__}_{date}_noise={noise_type}_reward={best_reward:.2f}_{GNN().__class__.__name__}_rew_eng={rew_eng}",
        rews_buf,
    )

    # save agent
    with open(
        f"./saved_agents/DDPGAgent_{CktGraph().__class__.__name__}_{date}_noise={noise_type}_reward={best_reward:.2f}_{GNN().__class__.__name__}_rew_eng={rew_eng}.pkl",
        "wb",
    ) as agent_file:
